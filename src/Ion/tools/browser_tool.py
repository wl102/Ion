...
def _is_tool_internal_sink(s: dict) -> bool:
    """Filter out eval calls made by Playwright internals or the tool itself."""
    if s.get("sink") != "eval":
        return False
    val = str(s.get("value", ""))
    return any(marker in val for marker in (
        "window.__ion_sinks",
        "JSHandle@",
        "injected.previewNode",
    ))


def _filter_tool_sinks(sinks: list[dict]) -> list[dict]:
    """Remove sink events generated by the tool / Playwright runtime."""
    return [s for s in sinks if not _is_tool_internal_sink(s)]
...
async def _browser_execute(
    ...
    try:
        ...
        # ---- dedupe, filter tool-internal sinks, & cap ----
        sink_events = _dedupe_sinks(accumulated_sinks)
        sink_events = _filter_tool_sinks(sink_events)
