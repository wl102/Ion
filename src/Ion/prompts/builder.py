...
| **XSS** | 1. `http_request` detects input reflected in response body/headers. <br> 2. `browser_execute(url=..., hook_xss_sinks=true)` navigates to the target (or submits via `actions`). <br> 3. `xss_sinks` array is NON-EMPTY OR `dialogs` contains alert/confirm/prompt. <br> 4. `summary.verdict == "exploit_triggered"`. | Reflected in raw HTML but no sink hits in browser → payload is HTML-encoded, inside a sandbox, or blocked by CSP. NOT a confirmed XSS. Stop immediately. |
